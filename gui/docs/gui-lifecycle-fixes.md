...
视频格式页的“快速选择分辨率”与格式输入框使用同一个 `StringVar`。选择项提交后由三条路径覆盖格式：变量变更、`<<ComboboxSelected>>` 事件，以及 macOS 原生只更新控件显示值时的鼠标释放后回调。最后一条路径通过 `after_idle` 读取已提交的下拉值，避免界面显示与实际 `-f` 参数不一致。

手动验证方式：启动 `gui/run_gui.sh`，进入“视频格式”，选择 `480p`，格式框必须立即显示 `bv*[height<=480]+ba`；选择 `1080p` 时应显示 `bv*[height<=1080]+ba`。
